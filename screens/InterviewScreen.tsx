      ...
      {!isSessionActive ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-8">
           ...
           <div className="text-center space-y-2">
              ...
              <p className="text-sm text-text-secondary max-w-xs">Waiting for you to enter the virtual room.</p>
           </div>
        ...
        <main className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-6 p-6 overflow-hidden min-h-0">
          ...
          <section className="lg:col-span-6 flex flex-col bg-[#1c212b] rounded-[32px] border border-white/5 shadow-2xl overflow-hidden relative min-h-0">
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                {(liveSarahText || liveUserText) && (
                   <div className={`flex flex-col ${liveUserText ? 'items-end' : 'items-start'} opacity-60`}>
                      <div className={`p-5 rounded-2xl max-w-[85%] italic text-sm ${liveUserText ? 'bg-primary/20' : 'bg-white/5'}`}>
                         {liveSarahText || liveUserText}
              ...
              <div className="h-20 bg-black/40 backdrop-blur-md border-t border-white/5 flex items-center px-8 gap-6 shrink-0">
                <div className={`size-10 rounded-full flex items-center justify-center border transition-all ${isMuted ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-primary/10 border-primary/20 text-primary'}`}>
                ...
                <div className="flex-1 flex items-center gap-1.5 h-6 justify-center">
                    {[...Array(24)].map((_, i) => (
                      <div key={i} className={`w-1 bg-primary rounded-full transition-all duration-300 ${!isMuted && !isPaused ? 'animate-pulse' : 'h-1'}`} 
                           style={{ height: !isMuted && !isPaused ? `${40 + Math.random() * 60}%` : '4px', animationDelay: `${i * 0.05}s` }}></div>
                    ))}
                </div>
                <div className="text-[10px] font-black text-text-secondary uppercase tracking-widest w-24 text-right">
