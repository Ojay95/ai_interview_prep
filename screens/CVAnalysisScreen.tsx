  ...
  return (
    <div className="flex flex-col h-screen bg-[#0d111a] text-white font-display overflow-hidden">
      <header className="flex items-center justify-between px-4 lg:px-10 py-3 border-b border-white/5 bg-[#0d111a]/80 backdrop-blur-md shrink-0 z-50">
        <div className="flex items-center gap-3 md:gap-4" onClick={() => onNavigate(Screen.Dashboard)}>
          <Logo className="size-7 md:size-8" />
          <h1 className="hidden sm:block text-sm md:text-lg font-bold tracking-tight cursor-pointer">MockInterview.ai</h1>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto space-y-6 md:space-y-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
               <button onClick={() => onNavigate(Screen.CVLanding)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl bg-white/5 border border-white/10 text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-colors">
               ...
               <button onClick={() => onNavigate(Screen.CVEditor)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl bg-primary text-white text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/20 hover:bg-primary-hover transition-colors">
                ...
                {analysisResult ? (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6 md:space-y-8">
                         ...
                         <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
                            <div className="relative size-28 md:size-40 shrink-0">
                               ...
                               <div className="absolute inset-0 flex flex-col items-center justify-center">
                                  ...
                                  <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">Match</span>
                               </div>
                            </div>
                            <div className="space-y-4 text-center md:text-left">
                               <div className="flex flex-col md:flex-row md:items-center gap-3">
                                  <h3 className="text-xl md:text-3xl font-black">Should you apply?</h3>
                                  <span className="px-3 py-1 rounded-full bg-primary/20 text-primary text-[10px] font-black uppercase tracking-widest border border-primary/30 w-fit mx-auto md:mx-0">
                      ...
                      <div className="space-y-6 md:space-y-8 pt-8 md:pt-10 border-t border-white/5">
                        <div className="flex items-center justify-between">
                           <div>
                              <h4 className="text-xl md:text-2xl font-black tracking-tight">AI Resume Optimization</h4>
                              <p className="text-text-secondary text-xs md:text-sm">Strategic edits to boost your ATS score and recruiter appeal.</p>
                           </div>
                           <button onClick={() => onNavigate(Screen.CVEditor)} className="hidden md:flex items-center gap-2 text-primary font-bold text-sm hover:underline">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                           {analysisResult.recommendations?.map((rec: any, i: number) => (
                             <div key={i} className="bg-[#1c212b] p-6 md:p-8 rounded-[24px] md:rounded-[32px] border border-white/5 group hover:border-primary/30 transition-all flex flex-col h-full shadow-lg">
                                <div className="flex items-center justify-between mb-6">
                                   <div className="flex items-center gap-4">
                                      <div className={`size-12 rounded-2xl flex items-center justify-center shrink-0 ${rec.impact === 'High' ? 'bg-primary/20 text-primary border border-primary/20' : 'bg-white/5 text-text-secondary'}`}>
                                         <span className="material-symbols-outlined text-xl">rocket_launch</span>
                                      </div>
                                      <div className="min-w-0">
                                         <h5 className="font-bold text-sm md:text-base truncate">{rec.title}</h5>
                                         <div className="flex items-center gap-2 mt-0.5">
                                            ...
                                            <span className="text-[9px] text-text-secondary opacity-20">•</span>
                                            <span className="text-[9px] text-text-secondary font-bold uppercase">ATS Optimized</span>
                                         </div>
                                      </div>
                                   </div>
                                </div>
                                <p className="text-xs md:text-sm text-text-secondary leading-relaxed mb-6 flex-1">
                                   {rec.description}
                                </p>
                                <div className="p-4 md:p-5 rounded-2xl bg-black/40 border border-white/5 text-xs md:text-sm text-white/90 relative group-hover:bg-primary/5 group-hover:border-primary/20 transition-all">
                                   <div className="absolute top-0 right-4 -translate-y-1/2 bg-[#1c212b] px-2 py-0.5 text-[8px] font-black uppercase tracking-widest text-primary border border-primary/20 rounded-full">Suggested Edit</div>
